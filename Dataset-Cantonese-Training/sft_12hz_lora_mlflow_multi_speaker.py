...
def build_speaker_embedding(speaker_model, ref_audio_path: str) -> torch.Tensor:
    if export_dataset is None:
        raise RuntimeError("export_dataset not initialized")
...
def train():
    ...
    with mlflow.start_run(run_name=f"lora-multi-speaker-r{args.lora_rank}-bs{args.batch_size}-warmup{args.warmup_ratio}"):
        ...
        def build_export_speaker_embeddings(speaker_model):
            return {
                speaker_id: build_speaker_embedding(
                    speaker_model,
                    first_ref_audio_map[speaker_id],
                )
        ...
        for epoch in range(args.num_epochs):
            ...
            if accelerator.is_main_process and num_batches > 0:
                ...
                speaker_embeddings = build_export_speaker_embeddings(speaker_model)

                speaker_model.merge_adapter()

                state_dict = clean_merged_state_dict(speaker_model)
                ...
                inject_speaker_embeddings(state_dict, speaker_slot_map, speaker_embeddings)
        ...
        if accelerator.is_main_process:
            ...
            speaker_model = accelerator.unwrap_model(model)
            speaker_embeddings = build_export_speaker_embeddings(speaker_model)
            clean_model = speaker_model.merge_and_unload()
            ...
            speaker_model.save_pretrained(os.path.join(final_dir, "lora_adapter"))
